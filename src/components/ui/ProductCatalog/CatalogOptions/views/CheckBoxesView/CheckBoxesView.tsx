...
const CheckBoxesView: FC<Props> = ({ data, onChangeData, selectedCheckbox }) => {
	if (data) {
		return (
			<div className={styles.checkbox__list}>
				{data.map(item => (
					<CatalogCheckBox key={item.name} id={item.name} label1={item.name} label2={item?.count} value={item.name === selectedCheckbox} callback={() => onChangeData(item.name)} />
				))}
			</div>
		);
	}
